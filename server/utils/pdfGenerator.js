 ...
 * @param {string} data.experimentDate
 * @param {string} data.requestingLab
 * @param {number} data.macswellSlides
 * @param {number} data.totalCocktailVolume
 * @param {string} [data.experimentType]
 * @param {Array}  data.antibodies - [{ tube_number, target, clone, fluorochrome, lab_name, titration_ratio, ul_per_slide, total_ul_used, chf_per_ul, total_chf }]
 * @param {number} data.totalCost
 * @returns {PDFDocument}
 */
exports.generateQuotePdf = (data) => {
  ...
  doc.text(`Date: ${data.experimentDate}`);
  doc.text(`Experiment: ${data.experimentName}`);
  if (data.experimentType) doc.text(`Type: ${data.experimentType}`);
  doc.text(`Requesting Lab: ${data.requestingLab}`);
  doc.text(`MACSwell Slides: ${data.macswellSlides}`);
